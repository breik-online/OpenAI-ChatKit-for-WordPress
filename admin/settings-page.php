<div class="wrap">
                ...
                <tr>
                    ...
                    <td>
                        <input type="password" id="chatkit_api_key" name="chatkit_api_key"
                               ...
                               class="regular-text"
                               placeholder="sk-proj-..."
                               <?php if (!empty($options['api_key_from_constant'])): ?>disabled<?php endif; ?>>
                        <?php if (!empty($options['api_key_from_constant'])): ?>
                        <p class="description" style="color: #00a32a;">
                            &#9989; <?php esc_html_e('API key is securely loaded from wp-config.php constant.', 'chatkit-wp'); ?>
                        </p>
                        <?php elseif (!empty($options['api_key'])): ?>
                        <div style="background: #fcf0f1; border-left: 3px solid #d63638; padding: 10px; margin-top: 8px;">
                            <strong style="color: #d63638;">&#9888; <?php esc_html_e('Security Warning:', 'chatkit-wp'); ?></strong>
                            <?php esc_html_e('Your API key is stored in the database. For better security, move it to wp-config.php:', 'chatkit-wp'); ?><br>
                            <code>define('CHATKIT_OPENAI_API_KEY', 'sk-proj-...');</code>
                        </div>
                        <?php else: ?>
                        <p class="description">
                            <?php esc_html_e('For security, add this key to wp-config.php:', 'chatkit-wp'); ?><br>
                            <code>define('CHATKIT_OPENAI_API_KEY', 'sk-proj-...');</code>
                        </p>
                        <?php endif; ?>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="chatkit_workflow_id" name="chatkit_workflow_id"
                               value="<?php echo esc_attr($options['workflow_id']); ?>"
                               class="regular-text"
                               placeholder="wf_...">
                        <p class="description">
                    ...
                    <td>
                        <label style="display:block; margin-bottom:12px;">
                            <input type="checkbox" name="chatkit_show_everywhere"
                                   <?php checked($options['show_everywhere'], true); ?>>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="chatkit_close_text" name="chatkit_close_text"
                               value="<?php echo esc_attr($options['close_text']); ?>"
                               class="regular-text"
                               placeholder="✕">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <select name="chatkit_accent_level">
                            <option value="0" <?php selected($options['accent_level'] ?? '2', '0'); ?>>0 - <?php esc_html_e('Subtle', 'chatkit-wp'); ?></option>
                            <option value="1" <?php selected($options['accent_level'] ?? '2', '1'); ?>>1 - <?php esc_html_e('Light', 'chatkit-wp'); ?></option>
                            <option value="2" <?php selected($options['accent_level'] ?? '2', '2'); ?>>2 - <?php esc_html_e('Normal', 'chatkit-wp'); ?></option>
                            <option value="3" <?php selected($options['accent_level'] ?? '2', '3'); ?>>3 - <?php esc_html_e('Bold', 'chatkit-wp'); ?></option>
                        </select>
                ...
                <tr>
                    ...
                    <td>
                        <select id="chatkit_theme_mode" name="chatkit_theme_mode">
                            <option value="dark" <?php selected($options['theme_mode'], 'dark'); ?>>Dark</option>
                            <option value="light" <?php selected($options['theme_mode'], 'light'); ?>>Light</option>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select id="chatkit_button_size" name="chatkit_button_size">
                            <option value="small" <?php selected($options['button_size'], 'small'); ?>><?php esc_html_e('Small', 'chatkit-wp'); ?></option>
                            <option value="medium" <?php selected($options['button_size'], 'medium'); ?>><?php esc_html_e('Medium', 'chatkit-wp'); ?></option>
                            <option value="large" <?php selected($options['button_size'], 'large'); ?>><?php esc_html_e('Large', 'chatkit-wp'); ?></option>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select id="chatkit_button_position" name="chatkit_button_position">
                            <option value="bottom-right" <?php selected($options['button_position'], 'bottom-right'); ?>><?php esc_html_e('Bottom Right', 'chatkit-wp'); ?></option>
                            <option value="bottom-left" <?php selected($options['button_position'], 'bottom-left'); ?>><?php esc_html_e('Bottom Left', 'chatkit-wp'); ?></option>
                            <option value="top-right" <?php selected($options['button_position'], 'top-right'); ?>><?php esc_html_e('Top Right', 'chatkit-wp'); ?></option>
                            <option value="top-left" <?php selected($options['button_position'], 'top-left'); ?>><?php esc_html_e('Top Left', 'chatkit-wp'); ?></option>
                        </select>
                ...
                <tr>
                    ...
                    <td>
                        <select id="chatkit_border_radius" name="chatkit_border_radius">
                            <option value="square" <?php selected($options['border_radius'], 'square'); ?>><?php esc_html_e('Square', 'chatkit-wp'); ?></option>
                            <option value="round" <?php selected($options['border_radius'], 'round'); ?>><?php esc_html_e('Round', 'chatkit-wp'); ?></option>
                            <option value="extra-round" <?php selected($options['border_radius'], 'extra-round'); ?>><?php esc_html_e('Extra Round', 'chatkit-wp'); ?></option>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select id="chatkit_shadow_style" name="chatkit_shadow_style">
                            <option value="subtle" <?php selected($options['shadow_style'], 'subtle'); ?>><?php esc_html_e('Subtle', 'chatkit-wp'); ?></option>
                            <option value="normal" <?php selected($options['shadow_style'], 'normal'); ?>><?php esc_html_e('Normal', 'chatkit-wp'); ?></option>
                            <option value="bold" <?php selected($options['shadow_style'], 'bold'); ?>><?php esc_html_e('Bold', 'chatkit-wp'); ?></option>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select id="chatkit_density" name="chatkit_density">
                            <option value="compact" <?php selected($options['density'] ?? 'normal', 'compact'); ?>><?php esc_html_e('Compact', 'chatkit-wp'); ?></option>
                            <option value="normal" <?php selected($options['density'] ?? 'normal', 'normal'); ?>><?php esc_html_e('Normal', 'chatkit-wp'); ?></option>
                            <option value="comfortable" <?php selected($options['density'] ?? 'normal', 'comfortable'); ?>><?php esc_html_e('Comfortable', 'chatkit-wp'); ?></option>
                        </select>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="chatkit_locale" name="chatkit_locale"
                               value="<?php echo esc_attr($options['locale'] ?? ''); ?>"
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="chatkit_greeting_text" name="chatkit_greeting_text"
                               value="<?php echo esc_attr($options['greeting_text']); ?>"
                               class="regular-text"
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" name="chatkit_default_prompt_<?php echo $i; ?>"
                               value="<?php echo esc_attr($options['default_prompt_' . $i]); ?>"
                        ...
                        <input type="text" name="chatkit_default_prompt_<?php echo $i; ?>_text"
                               value="<?php echo esc_attr($options['default_prompt_' . $i . '_text']); ?>"
                        ...
                        <select name="chatkit_default_prompt_<?php echo $i; ?>_icon">
                            <option value="circle-question" <?php selected($options['default_prompt_' . $i . '_icon'] ?? 'circle-question', 'circle-question'); ?>>❓ <?php esc_html_e('Question', 'chatkit-wp'); ?></option>
                            <option value="search" <?php selected($options['default_prompt_' . $i . '_icon'] ?? 'circle-question', 'search'); ?>>🔍 <?php esc_html_e('Search', 'chatkit-wp'); ?></option>
                            <option value="write" <?php selected($options['default_prompt_' . $i . '_icon'] ?? 'circle-question', 'write'); ?>>✍️ <?php esc_html_e('Write', 'chatkit-wp'); ?></option>
                            <option value="home" <?php selected($options['default_prompt_' . $i . '_icon'] ?? 'circle-question', 'home'); ?>>🏠 <?php esc_html_e('Home', 'chatkit-wp'); ?></option>
                            <option value="info" <?php selected($options['default_prompt_' . $i . '_icon'] ?? 'circle-question', 'info'); ?>>ℹ️ <?php esc_html_e('Info', 'chatkit-wp'); ?></option>
                        </select>
                        <?php if ($i === 1): ?>
                    ...
                    <td>
                        ...
                        <label style="display:block; margin-bottom:12px;">
                            <input type="checkbox" name="chatkit_enable_attachments"
                                   <?php checked($options['enable_attachments'], true); ?>>
                        ...
                        <input type="number" name="chatkit_attachment_max_size"
                               value="<?php echo esc_attr($options['attachment_max_size'] ?? '20'); ?>"
                        ...
                        <input type="number" name="chatkit_attachment_max_count"
                               value="<?php echo esc_attr($options['attachment_max_count'] ?? '3'); ?>"
                    ...
                    <td>
                        <label style="display:block;">
                            <input type="checkbox" name="chatkit_persistent_sessions"
                                   <?php checked($options['persistent_sessions'], true); ?>>
                    ...
                    <td>
                        <label style="display:block; margin-bottom:8px;">
                            <input type="checkbox" name="chatkit_show_header"
                                   <?php checked($options['show_header'] ?? true, true); ?>>
                            <?php esc_html_e('Show header', 'chatkit-wp'); ?>
                        </label>
                        <label style="display:block;">
                            <input type="checkbox" name="chatkit_show_history"
                                   <?php checked($options['show_history'] ?? true, true); ?>>
                    ...
                    <td>
                        <label style="display:block;">
                            <input type="checkbox" name="chatkit_hide_sources"
                                   <?php checked($options['hide_sources'] ?? false, true); ?>>
                    ...
                    <td>
                        ...
                        <div style="margin-bottom:20px;">
                            ...
                            <input type="text" name="chatkit_header_title_text"
                                   value="<?php echo esc_attr($options['header_title_text'] ?? ''); ?>"
                        ...
                        <div style="margin-bottom:20px;">
                            ...
                            <select name="chatkit_header_left_icon" style="margin-bottom:8px;">
                                ...
                                <option value="menu" <?php selected($options['header_left_icon'] ?? '', 'menu'); ?>><?php esc_html_e('☰ Menu', 'chatkit-wp'); ?></option>
                                <option value="settings-cog" <?php selected($options['header_left_icon'] ?? '', 'settings-cog'); ?>><?php esc_html_e('⚙️ Settings', 'chatkit-wp'); ?></option>
                                <option value="home" <?php selected($options['header_left_icon'] ?? '', 'home'); ?>><?php esc_html_e('🏠 Home', 'chatkit-wp'); ?></option>
                            </select>
                            <input type="url" name="chatkit_header_left_url"
                                   value="<?php echo esc_attr($options['header_left_url'] ?? ''); ?>"
                        ...
                        <div>
                            ...
                            <select name="chatkit_header_right_icon" style="margin-bottom:8px;">
                                <option value=""><?php esc_html_e('None', 'chatkit-wp'); ?></option>
                                <option value="home" <?php selected($options['header_right_icon'] ?? '', 'home'); ?>><?php esc_html_e('🏠 Home', 'chatkit-wp'); ?></option>
                                <option value="settings-cog" <?php selected($options['header_right_icon'] ?? '', 'settings-cog'); ?>><?php esc_html_e('⚙️ Settings', 'chatkit-wp'); ?></option>
                                <option value="menu" <?php selected($options['header_right_icon'] ?? '', 'menu'); ?>><?php esc_html_e('☰ Menu', 'chatkit-wp'); ?></option>
                            </select>
                            <input type="url" name="chatkit_header_right_url"
                                   value="<?php echo esc_attr($options['header_right_url'] ?? ''); ?>"
                                   class="regular-text" placeholder="https://example.com">
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <textarea name="chatkit_disclaimer_text" rows="3" class="large-text"
                                  placeholder="<?php esc_attr_e('AI can make mistakes. Check important info.', 'chatkit-wp'); ?>"><?php echo esc_textarea($options['disclaimer_text'] ?? ''); ?></textarea>
                        ...
                        <label style="display:block; margin-top:10px;">
                            <input type="checkbox" name="chatkit_disclaimer_high_contrast"
                                   <?php checked($options['disclaimer_high_contrast'] ?? false, true); ?>>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="chatkit_initial_thread_id" name="chatkit_initial_thread_id"
                               value="<?php echo esc_attr($options['initial_thread_id'] ?? ''); ?>"
    ...
    document.getElementById('chatkit-test-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            notice.className = response.ok ? 'notice notice-success inline' : 'notice notice-error inline';
            const p = document.createElement('p');
            p.textContent = response.ok ? ('\u2705 ' + data.message) : ('\u274C ' + (data.message || 'Unknown error'));
